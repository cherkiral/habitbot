...
export default function ActivityPage() {
  ...
  const stepsPct = stats?.steps_goal
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px 24px', background:c.bg }}>
        <div style={{ maxWidth:1200, margin:'0 auto', display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div style={card()}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginBottom:12 }}>
                <div>
                  ...
                  <input style={inp()} type="number" placeholder="5000" value={form.steps}
                    onChange={e => setForm(f => ({ ...f, steps:e.target.value }))} />
                </div>
                <div>
                  <p style={{ fontSize:11, color:c.muted, marginBottom:5 }}>Длительность (мин)</p>
                  <input style={inp()} type="number" placeholder="30" value={form.duration_min}
                    onChange={e => setForm(f => ({ ...f, duration_min:e.target.value }))} />
                </div>
                <div>
                  <p style={{ fontSize:11, color:c.muted, marginBottom:5 }}>Интенсивность</p>
                  <select style={{ ...inp(), cursor:'pointer' }} value={form.intensity}
              ...
              <input style={inp({ marginBottom:12 })} type="text" placeholder="Заметка (необязательно)"
          ...
          <div style={card()}>
            ...
            {!logs?.length && (
              <p style={{ fontSize:13, color:c.hint, textAlign:'center', padding:'20px 0' }}>Нет тренировок</p>
            )}
                  ...
                  <div key={l.id} style={{ display:'flex', gap:12, padding:14, background:c.bg, borderRadius:10, border:`1px solid ${c.border}` }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6 }}>
                      ...
                      <button onClick={() => del.mutate(l.id)}
                        style={{ background:'none', border:'none', cursor:'pointer', color:'#e88', fontSize:12 }}>✕</button>
                    </div>
                  </div>
                )
              })}
