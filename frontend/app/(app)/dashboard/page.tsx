    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div>
          ...
          {weightStats?.target_weight_kg && weightStats?.current_weight_kg && (
            <p className="text-sm text-muted mt-1">
              До цели осталось{' '}
              <span className="text-secondary font-medium">
                {Math.max(0, weightStats.current_weight_kg - weightStats.target_weight_kg).toFixed(1)} кг
              </span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            unit={foodSummary?.calories_goal ? /  : 'ккал'}
            delta={foodSummary?.calories_remaining
              ? Осталось  ккал
              : undefined}
            deltaType="neutral"
          />
          <StatCard
            ...
            unit="кг"
            delta={weightStats?.delta_week_kg
              ? ${weightStats.delta_week_kg > 0 ? '+' : ''} кг за неделю
              : undefined}
          ...
          <StatCard
            ...
            unit={activityStats?.steps_goal ? / k : ''}
            delta={activityStats?.steps_progress_pct
              ? ${activityStats.steps_progress_pct}% от цели
              : undefined}
            deltaType="neutral"
          />
          <StatCard
            ...
            <div className="flex gap-1.5">
              {Array.from({ length: 7 }).map((_, i) => (
                <div key={i} className="w-2 h-2 rounded-full"
                  style={{
                    background: i < (streaks.food.current_streak % 7) ? '#d97706' : 'rgba(255,255,255,0.2)'
                  }} />
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card>
            ...
            <div className="flex gap-2">
              {[150, 250, 500].map(ml => (
                <Button key={ml} variant="ghost" size="sm" className="flex-1"
                  onClick={() => addWater(ml)}>
                  +{ml} мл
                </Button>
              ))}
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={label} className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-mono text-hint w-16 text-right">
